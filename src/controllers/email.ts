...
class Email {
	...
	static async send(email: IEmail) {
		...
		console.log(mcEmail.personalizations[0])
		...
		if (resp.status > 299 || resp.status < 200) {
			const errorBody = await resp.text(); // 或者 resp.json()，如果错误响应是JSON格式的
			throw new Error(`${resp.status} ${resp.statusText}: ${errorBody}`);
		}
